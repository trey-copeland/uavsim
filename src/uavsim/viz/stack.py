    ...
    if t == "ndi_cascade":
        return _equations_payload(
            ...
            [
                ...
                "  e_p = p_r − p,   e_v = v_r − v",
                "  a_des = a_r + Kp_pos ⊙ e_p + Kd_pos ⊙ e_v",
                "    a_r from ReferenceSample.a_ref (sampled traj ∇v) when present, else 0",
                "  tilt safety: clamp a so f=m(a−g e_z) has f_z<0 and",
                "    tilt(b₃_des) ≤ max_tilt_rad  (cone about vertical; no inverted R_des)",
                "Collective thrust + desired attitude (NED z+ down):",
                "  f_i = m (a_des − g e_z)   # inertial thrust force",
                "  F = clip(‖f_i‖, F_min, F_max),   b₃_des ∥ −f_i   (‖·‖ not −f·(R e₃))",
                "  R_des from b₃_des and ψ_r (yaw)",
                "SO(3) + rate NDI (Euler equation inverse):",
                "  e_R = vee(log(R_desᵀ R)),   e_ω = ω − ω_des",
                "  ω_des = x_r[pqr] when use_ref_omega (memoryless; no Ṙ finite-diff)",
